...
#endif
	work_scale = std::min(1.0, sqrt(registration_resol * 1e6 / full_img[0].size().area()));
	double seam_scale = std::min(1.0, sqrt(seam_estimation_resol * 1e6 / full_img[0].size().area()));
	seam_work_aspect = seam_scale / work_scale;
	int num_features = int((work_scale * work_scale * full_img[0].size().area()) / 100);
#if ON_LOGGER
	printf("%d\n", num_features);
#endif
	cv::Ptr<cv::detail::FeaturesFinder> finder = new cv::detail::OrbFeaturesFinder(cv::Size(3, 1), num_features, 1.3f, 5);
	...
	std::vector<cv::detail::MatchesInfo>& pairwise_matches) 
{
	...
	std::vector<int> indices = leaveBiggestComponent(features, pairwise_matches, confidence_threshold);
...
#pragma omp parallel for
	for (int img_idx = 0; img_idx < num_images; ++img_idx) 
	{
		...
		if (abs(compose_scale - 1) > 1e-1)
		{
			cv::resize(full_img[img_idx], img[img_idx], cv::Size(), compose_scale, compose_scale);
		}
		else
		{
...
#pragma omp parallel for
		...
		{
			matching_mask.at<unsigned char>(pairwise[i].first, pairwise[i].second) = 1;
		}
	}
}
*/
	...
	if (status.first != OK)
	{
		...
		switch (status.first) 
		{
			...
			case OK:
				...
				break;
			case NOT_ENOUGH:
				if (tmp_code.first == status.first && tmp_code.second < status.second) 
				{
					result = retry.clone();
				}
				else 
